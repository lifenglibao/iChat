    ...
    internal func socketCMDStatus (cmd:String,object:AnyObject) {
        if (appDelegate.webSocket.readyState == .OPEN){
            ...
            if (cmd == CMD_CREATE_GROUP_CHAT) {
                ...
                temp.append(NSNumber.init(integer: appDelegate.SELF_USER_ID).stringValue)
                let invite_id = temp.map{String($0)}.joinWithSeparator(",")
                print(invite_id)
                appDelegate.webSocket.send(CREATE_GROUP_CHAT_CMD("test", CHANNAL_ID: 0, SELFS_ID: appDelegate.SELF_USER_ID, INVITE_ID:invite_id))
            }
