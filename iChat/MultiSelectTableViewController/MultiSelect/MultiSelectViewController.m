...
- (void) initNavBar {
    ...
    self.navigationController.navigationBar.barTintColor = RGBALPHA(0.071, 0.060, 0.086, 1.0);
...
- (void)viewDidLoad
{
    ...
    for (MultiSelectItem *item in self.items) {
        NSString *firstLetter = [self firstLetterOfString:item.friend_name];
    ...
    for (NSString *key in [self.dict allKeys]) {
        if (((NSArray*)self.dict[key]).count<=0) {
            ...
            self.dict[key] = [((NSArray*)self.dict[key]) sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
                return [((MultiSelectItem *)obj1).friend_name localizedCompare:((MultiSelectItem *)obj2).friend_name];
            }];
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:self.tableView]) {
        ...
        [cell.cellImageView setImageWithURL:item.friend_avatar];
        cell.label.text = item.friend_name;
        if (item.disabled) {
    ...
    [cell.cellImageView setImageWithURL:item.friend_avatar];
    cell.contentLabel.text = item.friend_name;
    if (item.disabled) {
    ...
    for (MultiSelectItem *item in self.selectedItems) {
        ...
        temp = [NSMutableArray arrayWithObjects:
                [NSDictionary dictionaryWithObjects:
                 [NSArray arrayWithObjects:item.friend_id,item.friend_name,item.friend_avatar, nil]
